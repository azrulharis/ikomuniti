...
class IsahabatactivationsController extends ControllerBase {
	
	public function initialize() {
		$this->tag->setTitle('iSahabat activation');
		parent::initialize();
	}
	...
	public function allAction() {
	    ...
	    $this->view->setVar('users', $this->get_user($auth['id']));
	    if($auth['role'] <= 1) {
			$this->flash->error('No iSahabat to activate');
		} else {
	...
	private function problem($id) {
		...
		if($user->save()) {
			...
			return $this->response->redirect('isahabatactivations/problems');
		}
	}
	...
	*/	
	private function check_epin($user_id, $used_user_id) {
		$phql = "SELECT id FROM JunMy\Models\Isahabatepins WHERE user_id = '$user_id' AND used_user_id = '0' LIMIT 1";
		$rows = $this->modelsManager->executeQuery($phql);
		if(count($rows) > 0) {
			$update = "UPDATE JunMy\Models\Isahabatepins SET 
		    ...
		    if($return) {
		        if($this->activate_user($used_user_id)) {
		            ...
		            if($this->update_counter($user->username_sponsor, $used_user_id)) {
						...
						return $this->response->redirect('isahabatactivations/index');
					} 
				} else {
...
Thank you very much for your priceless support. Please visit www.ishare.com.my and Like! www.facebook.com/ishare.com.my for more info.';
	...
	*  Return Update (iSahabat = 3 iKomuniti = 4, iReseller = 5)
	*/	
	private function activate_user($id) {
	    $date = date('Y-m-d H:i:s');	
	    $phql = "UPDATE JunMy\Models\Users SET 
		role = '3', verified = '1', created = '$date' WHERE id = '$id'";
