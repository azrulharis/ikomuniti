...
class IsahabatpinsController extends ControllerBase {
	...
	public function initialize() { 
		$this->tag->setTitle('iSahabat iPins');
		parent::initialize();
	}
	...
	public function transferAction() {
	    ...
	    if($this->request->isPost()) {
            ...
            if(!is_numeric($count)) { 
                // Check epin balance
				$this->flash->error('Please enter numeric format on total iPin'); 	 
			...
			if($_GET['proceed'] == 'Batal') {
				...
				if(count($this->select_epin_token($token)) > 0) { 
				    ...
				    if($this->update_epin($receiver_id, $sender_id, $sender_username, $total_epin, $token)) {
				        unset($_SESSION['csrf']);
						$this->flashSession->success('iSahabat iPin transfer has been done.');
						return $this->response->redirect('isahabatpins/transfer');
					} else {
	...
	*/
	private function select_epin($user_id) {
		$phql = "SELECT id FROM JunMy\Models\Isahabatepins WHERE user_id = '$user_id' AND used_user_id = '0'";
		$rows = $this->modelsManager->executeQuery($phql);
	...
	*  Update ePin user_id to receiver
    ...
    private function view_epin($id) {
		...
		$phql = "SELECT
		    *
			FROM JunMy\Models\Isahabatepins 
