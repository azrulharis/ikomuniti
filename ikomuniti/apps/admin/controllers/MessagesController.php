...
class MessagesController extends ControllerBase {
	...
	public function indexAction() { 
		...
		$key = 'gghadmin_messages_index_'.$auth['id'].'_'.$offset;
...
// Insert message. Used on index
	...
	public function viewAction() {
	    parent::pageProtect(); 
		$this->view->ajaxreply = $this->url->get('gghadmin/ajax/ajaxreply'); 
	    $auth = $this->session->get('junauth');
	    ...
	    $_SESSION['ARMTV'] = $this->passwordHash(date('YmdHis'));
	    $this->view->token = $_SESSION['ARMTV']; 
	    $this->view->user_id = $auth['id'];
