...
class ActivationsController extends ControllerBase {
	...
	public function problemsAction() {
		...
		$this->view->setVar('views', $this->view_user($auth['username'], 44));
		...
		if(isset($_GET['delete']) == 'delete' && isset($_GET['user_id'])) {
			$user_id = $_GET['user_id'];
			
			if(!is_numeric($user_id)) {
				$this->flash->error('Not valid user id');
			} else {
				$user = Users::count("id = '$user_id' AND verified='44'");
				if(!$user) {
					$this->flash->error('User not found. Make sure user has been save into problem area before delete it');
				} else {
					$this->remove($user_id);
				}
	...
	private function problem($id) { // 2621
		$user = Users::findFirst($id);
		$user->verified = 44;
		if($user->save()) {
			$this->flashSession->success('User has been add to problems area'); 
			return $this->response->redirect('gghadmin/activations/problems');
		}
	}
	
	private function remove($id) { // 2621
		$user = Users::findFirst($id);
		$user->verified = 45;
		if($user->save()) {
			$this->flashSession->success('User has been delete'); 
			return $this->response->redirect('gghadmin/activations/problems');
		}
	}
