...
use JunMy\Models\Isahabatepins;
use JunMy\Components\Pagination\Pagination;

class IsahabatpinsController extends ControllerBase {
	...
	public function initialize() {
		$this->tag->setTitle('iSahabat iPin');
		parent::initialize();
	}
		...
		if($this->request->isPost()) {
			if(is_numeric($this->request->getPost('count'))) {
				if($this->request->getPost('count') > 1) {
					...
					$this->flashSession->success('iSahabat iPin has been generated');
					return $this->response->redirect('gghadmin/isahabatpins/add');		
	...
	public function transferAction() {
		...
		if(isset($_GET['ref']) && isset($_GET['ntsv']) && isset($_GET['status']) && isset($_GET['proceed']) && isset($_GET['nt'])) {
			if($_GET['proceed'] == 'Batal') {
				$this->response->redirect('gghadmin/isahabatpins/transfer');
				...
				if(count($this->select_epin_token($token)) > 0) {
				    ...
				    if($this->update_epin($receiver_id, $sender_id, $sender_username, $total_epin, $token)) {
						$this->flashSession->success('iSahabat iPin has been transfer successfully');
						return $this->response->redirect('gghadmin/isahabatpins/transfer');
					} else {
	...
	*/
    private function select_epin_token($token) {
		$phql = "SELECT id FROM JunMy\Models\Isahabatepins WHERE token = '$token' LIMIT 1";
		$rows = $this->modelsManager->executeQuery($phql);
	...
	*/
	private function select_epin($user_id) {
		$phql = "SELECT id FROM JunMy\Models\Isahabatepins WHERE user_id = '$user_id'";
		$rows = $this->modelsManager->executeQuery($phql);
	...
	*/
	...
	private function track_epin($id) {
		...
		$phql = "SELECT 
			...
			FROM JunMy\Models\Isahabatepins AS e
	...
	*/
	private function update_epin($to_user_id, $from_user_id, $from_username, $limit, $token) {
	    $date = date('Y-m-d H:i:s');
		$phql = "UPDATE JunMy\Models\Isahabatepins SET 
				user_id = '$to_user_id', 
	...
	/*
	*  Generate iSahabat iPin length = 6
	*/
