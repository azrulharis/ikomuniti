  		...
  		<div class="panel-body">  
		     ...
		     {% for post in posts %}
               <div class="row">
	              ...
	              <a href="{{admin_path}}ipartner/view/{{post.url}}">
				  <div class="col-md-2 text-center">
				  {% if post.image != '' %}
				    {{ image('uploads/ipartners/images/'~post.image, 'title': post.title, 'alt': post.title, 'data-src': 'holder.js/500x500/auto', 'width':140)}} 
				  {% else %}
				    {{ image('uploads/ipartners/images/no_photo.jpg', 'title': post.title, 'alt': post.title, 'data-src': 'holder.js/500x500/auto', 'width':140)}} 
				  {% endif %}
				  </div>
				  ...
				  <div class="col-md-3 text-left" style="margin-top: 10px;">
				    {% if post.discount != 0 %}<i class="fa fa-star"></i> <b>{{post.discount}}</b>{% endif %}
				  </div> 
